...
public final class MainActivity extends Activity {
    ...
    private final ServiceConnection conn = new ServiceConnection() {
        @Override public void onServiceConnected(ComponentName n, IBinder b) {
            ...
            HidController controller=service.controller();
            if(controller!=null){controller.observe(observer);if(searchAfterBind){searchAfterBind=false;controller.search();}}
        }
    ...
    private void build(){
        ...
        editor=new EditText(this);editor.setHint("Write something…");editor.setGravity(Gravity.TOP|Gravity.START);editor.setMinLines(3);editor.setMaxLines(5);editor.setInputType(0x00004001|0x00020000);
    ...
    private void ensureService(boolean search){
        if(Build.VERSION.SDK_INT>=31&&checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)!=PackageManager.PERMISSION_GRANTED){searchAfterBind=search;requestPermissions(new String[]{Manifest.permission.BLUETOOTH_CONNECT},REQ);return;}
        try{startForegroundService(new Intent(this,KeyboardService.class));if(search)searchAfterBind=true;if(!bound)bound=bindService(new Intent(this,KeyboardService.class),conn,BIND_AUTO_CREATE);if(search&&service!=null&&service.controller()!=null){searchAfterBind=false;service.controller().search();}}
    ...
    private void render(HidController.Snapshot s){
        ...
        info.setText(s.message+"\nSearch lists paired Bluetooth computers; select your laptop, then Connect.");
        list.removeAllViews();
        for(HidController.DeviceRow d:s.devices){
            LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.VERTICAL);TextView name=new TextView(this);name.setText(d.name);name.setTextSize(17);row.addView(name);
            LinearLayout actions=new LinearLayout(this);Button sel=btn("Select Device"),c=btn("Connect"),x=btn("Disconnect");actions.addView(sel,new LinearLayout.LayoutParams(0,-2,1));actions.addView(c,new LinearLayout.LayoutParams(0,-2,1));actions.addView(x,new LinearLayout.LayoutParams(0,-2,1));row.addView(actions);
            sel.setOnClickListener(v->service.controller().select(d.address));c.setOnClickListener(v->service.controller().connect());x.setOnClickListener(v->service.controller().disconnect());list.addView(row);
        }
        editor.setEnabled(s.connected&&!s.busy);send.setEnabled(s.connected&&!s.live&&!s.busy);live.setEnabled(!s.busy);send.setText(s.live?"Send Buffer":"Send Buffer (use composer text)");
        if(s.connected&&!s.draft.equals(editor.getText().toString())){editor.setText(s.draft);editor.setSelection(editor.length());}
    }
