...
public final class MainActivity extends Activity {
    ...
    private final ServiceConnection conn = new ServiceConnection() {
        @Override public void onServiceConnected(ComponentName n, IBinder b) {
            ...
            if(controller!=null){
                ...
                if(searchAfterBind){searchAfterBind=false;controller.search();}
            }
        }
    ...
    private void build(){
        ...
        if(Build.VERSION.SDK_INT>=23){
            root.setOnApplyWindowInsetsListener((v,insets)->{
                ...
                root.setPadding(root.getPaddingLeft(),root.getPaddingTop(),root.getPaddingRight(),baseBottomPadding+bottom);
                return insets;
            });
        ...
        info=new TextView(this);root.addView(info);
        ...
        editor.setMinLines(3);editor.setMaxLines(5);
    ...
    private void render(HidController.Snapshot s){
        ...
        info.setText(s.message+"\nPaired Bluetooth computers are listed here. Select your laptop, then Connect.");
        list.removeAllViews();
        for(HidController.DeviceRow d:s.devices){
            LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.VERTICAL);
            TextView name=new TextView(this);name.setText(d.name);name.setTextSize(17);row.addView(name);
