...
public final class KeyboardService extends Service {
    ...
    private final BroadcastReceiver btReceiver=new BroadcastReceiver(){@Override public void onReceive(Context c,Intent i){if(controller==null)return;String a=i.getAction();if(BluetoothAdapter.ACTION_STATE_CHANGED.equals(a))controller.adapterChanged(i.getIntExtra(BluetoothAdapter.EXTRA_STATE,BluetoothAdapter.ERROR));else if(BluetoothDevice.ACTION_ACL_CONNECTED.equals(a))controller.aclChanged(device(i),true);else if(BluetoothDevice.ACTION_ACL_DISCONNECTED.equals(a))controller.aclChanged(device(i),false);}};
    ...
    private void registerReceiver(){IntentFilter f=new IntentFilter();f.addAction(BluetoothAdapter.ACTION_STATE_CHANGED);f.addAction(BluetoothDevice.ACTION_ACL_CONNECTED);f.addAction(BluetoothDevice.ACTION_ACL_DISCONNECTED);if(Build.VERSION.SDK_INT>=33)registerReceiver(btReceiver,f,Context.RECEIVER_EXPORTED);else registerReceiver(btReceiver,f);receiver=true;}
